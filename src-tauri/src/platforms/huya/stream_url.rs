...
// Same display format as live_list.rs: plain number until 1万, then "x.x万".
fn format_wan_count(n: i64) -> String {
    if n >= 10_000 {
...
fn extract_room_detail(profile: &Value) -> RoomDetail {
    ...
    let cover_url = data
        .get("liveData")
        .and_then(|ld| ld.get("screenshot"))
        .and_then(|x| x.as_str())
        .map(|s| s.to_string());

    let live_data = data.get("liveData");
    let audience_count = live_data
        .and_then(|ld| ld.get("audienceCount").or_else(|| ld.get("liveCount")))
        .map(|v| parse_i64_lossy(Some(v)))
        .unwrap_or(0);
    let viewer_count_str = if audience_count > 0 {
        Some(format_wan_count(audience_count))
    } else {
        None
    };
    ...
    let selected_index = match selection {
        ...
        None => {
            return Ok(HuyaUnifiedResponse {
                ...
                viewer_count_str: detail.viewer_count_str.clone(),
            });
        }
    };
